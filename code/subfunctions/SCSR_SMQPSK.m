function [x_est,arrSER] = SCSR_SMQPSK(y,A,arrC,matQ,invMat,lambda,rho,nIteration,x_true);
%SCSR_SMQPSK SCSR optimization for complex discrete-valued sparse vector
%
% Input
...
%
  ...
  for k=1:nIteration
    ...
    z=orig+(prox_real+1j*prox_imag).*(abs(orig)>0)+prox.*(abs(orig)==0);
    r=r+Phi*s-z;
