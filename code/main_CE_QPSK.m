%%--------------------------------------------------------------------------
% simulation for IW-SCSR in channel equalizaiton with cyclic prefix (QPSK)
%%--------------------------------------------------------------------------

addpath('subfunctions');

n=4; % number of transmit antennas
m=3; % number of receive antennas
L_path=5; % number of multipath
nBlock=32; % block length
N=n*nBlock;
M=m*nBlock;
...
for i=1:nSample
  disp(['i=' num2str(i)]);
  % channel matrix
  [A,~]=makeChannel_CE(m,n,L_path,nBlock);
  
  for SNRIndex=1:length(arrSNR)
    SNR=arrSNR(SNRIndex);
    disp(['  SNR=' num2str(SNR)]);
    % variance of additive noise
    sigma_c=sqrt(2*n*L_path/(10^(SNR/10)));
